...
% \prtodo{Incorporate:
%     While it is crucially important to distinguish when observations are a result of chance versus a real effect, statistics does not offer much guidance in determining which effects and which effect sizes are important.
% }
...
% \subsection{Ranking by Information Gain}
% \label{ch:isicle:info}
% \prtodoi{Consider cutting info gain}
% The foundational goal of science is to create new knowledge and in the context of \nlp{} this means finding methods with novel, interesting, and beneficial behavior.
% Given two models, it may be that they perform similarly on aggregate, but behave very differently in ways which most statistical comparisons would miss.
% Similarly, while one model may break state-of-the-art, its impact with regards to creating new knowledge is diminished if it is nearly identical to the prior model.
% One way to concisely capture these model differences is by evaluating the information gained by having access to a new model.
% To operationalize this intuition, we compute the Principal Component Analysis and explained variance scores $\sigma_j^2$ on the item-agent matrix where entry $e_{i,j}$ contains the score for item $i$ from agent $j$.
% After computing this, we examine the explained variance $\sigma_j^2$ of the proposed method.\footnote{
%     In a degenerate case with identical models, we reconcile by detecting duplicates.
% }
% In Section~\ref{ch:isicle:exp} we compare how explained variance correlates to statistical comparisons.

% One drawback with this particular operationalization is that it does not distinguish between explained variance that leads to a higher score versus a lower score.
% Rather than only using \abr{pca}, we also consider an idea inspired by model ensembles that incorporates both valuing a higher score and models providing unique predictions.
% Under this approach, we first ensemble the predictions of all known models minus the model under consideration.
% Second, we calculate the score of the ensemble when using an oracle that selects the best prediction for each example.
% Finally, we recompute this score, but include the model under consideration.
% The difference between the two scores (lift) represents the additive benefit of the model which we test as an alternative ranking metric.
...
\end{equation}
%\begin{align}
%    P_s & =\frac{(P+\delta)(1-P-\delta)}{(P+\delta)(P-\delta) + (P+\delta)(1-P+\delta) +  (1-P-\delta)(P-\delta) + (1-P-\delta)(1-P + \delta)} \\
%        & =\delta^2+\delta + P - P^2
%\end{align}
%in terms of $P$ and $\delta$.\prtodo{How to fix over flow?}
\noindent
